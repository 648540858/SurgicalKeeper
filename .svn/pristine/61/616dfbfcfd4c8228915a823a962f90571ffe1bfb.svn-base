...
public class WeiXinItfControllerV2 extends WeiXinItfBaseController {
	...
	@RequestMapping(value = "/querySubYyDoc")
	public String querySubYyDoc(HttpServletRequest request, HttpServletResponse response) throws Exception {
		List<?> data = super.getFredaReader().queryForList("WxV2.querySubYyDoc", WebUtil.getPraramsAsDto(request));
...
//				FredaContext.getInstance().getSmsService().sendSystemSMS(rdto.getAsString("_turename"), rdto.getAsString("_mobile"), 2, smscontent, 5);
				/* 告知客服 */
				String jsonString=request.getServletContext().getAttribute(SystemCons.APPLICATION_SYSTEM_CODE_VAR).toString();
				String mobile=BasicUtils.getParamVarCode(jsonString, "TIP_KF_SMSMOBILE");
				String openWeiXinId=BasicUtils.getParamVarCode(jsonString, "TIP_KF_WXID");
				if(mobile!=null&&mobile.length()==11){
					smscontent = "精准预约申请提醒：就诊人：" + rdto.getAsString("_turename") + "，请登录后台进行处理";
					FredaContext.getInstance().getSmsService().sendSystemSMS("专家客服", mobile, 3, smscontent, 5);
				}
				if(openWeiXinId!=null&&openWeiXinId.length()>0){
//					String kfMsg="{\"touser\":\""+openWeiXinId+"\",\"msgtype\":\"text\",\"text\":{\"content\":\""+smscontent+"\"}}";
//					WeiXinUtils.sendKfMsg(WeiXinTokenThread.ACCESS_TOKEN, kfMsg);
					String url=WeiXinCons.WX_SAFE_DOMAIN+"/WxPro/wx/orderTip.jsp";
					
					WxTemplate t = new WxTemplate(); 
		            t.setUrl(url);
		            t.setTouser(openWeiXinId);
		            ...
		            first.setValue("你好:就诊人:"+rdto.getAsString("_turename")+",快速精准预约以下医生:");  
		            m.put("first", first);
		            TemplateData td1 = new TemplateData();
		            td1.setColor("#173177");  
		            td1.setValue(rdto.getAsString("doctorName"));
		            m.put("keyword1", td1);
		            TemplateData td2 = new TemplateData();
		            td2.setColor("#173177");
		            td2.setValue(rdto.getAsString("hospName"));
		            m.put("keyword2", td2);
		            TemplateData tdr = new TemplateData();
		            tdr.setColor("#173177");  
		            tdr.setValue("请登录平台及时处理!");
		            m.put("remark", tdr);
		            t.setData(m);
					
					WeiXinUtils.sendTemplateMsg(WeiXinTokenThread.ACCESS_TOKEN,JsonHelper.encodeObject2Json(t));
				}
			}
	...
	public String addVipConsultation(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String openWeiXinId=BasicUtils.getParamVarCode(jsonString, "TIP_KF_WXID");
		if(mobile!=null&&mobile.length()==11){
			FredaContext.getInstance().getSmsService().sendSystemSMS("客服专员", mobile, 3, smscontent, 5);
		}
		if(openWeiXinId!=null&&openWeiXinId.length()>0){
//			String kfMsg="{\"touser\":\""+openWeiXinId+"\",\"msgtype\":\"text\",\"text\":{\"content\":\""+smscontent+"\"}}";
//			WeiXinUtils.sendKfMsg(WeiXinTokenThread.ACCESS_TOKEN, kfMsg);
            ...
            TemplateData first = new TemplateData();
            first.setColor("#173177");  
            first.setValue("你好:就诊人:"+pDto.getAsString("patName")+",快速VIP预约:");  
            m.put("first", first);
            TemplateData td1 = new TemplateData();
            td1.setColor("#173177");  
            td1.setValue("平台推荐");
	 ...
	 * @return: String
	 */
	@RequestMapping(value = "/queryMyDoc")
	public String queryMyDoc(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Dto dto = WebUtil.getPraramsAsDto(request);
		
		List<?> data = super.getFredaReader().queryForList("WxV2.queryMyDoc", dto);

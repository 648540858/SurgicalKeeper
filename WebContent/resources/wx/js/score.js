	...
	_bindEnv: function(parentCls){
		...
		$(_config.selector + ' li',parentCls).each(function(index,item){
			...
			$(item).mouseover(function(e){
				...
				var html = '<em>' + '<b>'+(index+1)+'</b>分 '+_cache.aMsg[index].split('|')[0]+'' + 
						   '</em>' + _cache.aMsg[index].split('|')[1];
				$('p',parentCls).html(html);

				!$('p',parentCls).hasClass('hidden') && $('p',parentCls).addClass('hidden');
				var html0 = '<strong>' +(index+1) + '分</strong>' +_cache.aMsg[index].split('|')[1];
				
				$('.desc',parentCls).html(html0);
				$('#level').val(index);
			...
			/*$(item).click(function(e){
				...
				!$('p',parentCls).hasClass('hidden') && $('p',parentCls).addClass('hidden');
				var html = '<strong>' +index + '分</strong>' +_cache.aMsg[index].split('|')[1];

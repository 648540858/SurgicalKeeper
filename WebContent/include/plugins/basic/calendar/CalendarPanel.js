...
Ext.define('Ext.calendar.CalendarPanel', {
    ...
    initComponent: function() {
        ...
        if (this.showMonthView) {
            ...
            //alert(month.store || this.eventStore);
            //month.store = this.eventStore;
    ...
    // private
    onDayClick: function() {
    	this.quickAddSchForMonthWinInit();
     ...
     * 快速添加窗口初始化
     */
    quickAddSchForMonthWinInit : function (){
    	var that = this;
  
		this.quickWin = Ext.create('Ext.window.Window',{
			layout : 'fit',
			width : 600,
			height : 220,
			resizable : false,
			...
			items : [Ext.create('Ext.form.Panel',{
				id : 'quickAddSchFormPanel',
				name : 'quickAddSchFormPanel',
				labelAlign : 'right',
				labelWidth : 110,
				frame : false,
				layout: 'anchor',
			    ...
			    items: [{
					...
					store : Ext.create('Ext.data.Store',{
						fields: ['name', 'code'],
						data : [{name : '快速预约' , code : 1},{name : '精准预约' , code : 2}]
					}),
					displayField: 'name',
	    			valueField: 'code'
			    },{
					...
					store : Ext.create('Ext.data.Store',{
						fields: ['name', 'code'],
						data : [{name : '上午(10点-12点)' , code : 1},{name : '下午(3点-5点)' , code : 2}]
					}),
					displayField: 'name',
	    			valueField: 'code'
			    },{
			        ...
			        items : [{
			        	...
			        	columnWidth:0.14,
			        	items: [  
		                    { boxLabel: '星期日', name: 'cb', inputValue: '1', checked: true},  
		                    { boxLabel: '星期一', name: 'cb', inputValue: '2' },  
		                    { boxLabel: '星期二', name: 'cb', inputValue: '3' },  
		                    { boxLabel: '星期三', name: 'cb', inputValue: '4' },  
		                    { boxLabel: '星期四', name: 'cb', inputValue: '5' },  
		                    { boxLabel: '星期五', name: 'cb', inputValue: '6' },
		                    { boxLabel: '星期六', name: 'cb', inputValue: '7' }
		                ]
			        }]
			    },{
			    	xtype : 'hiddenfield',
     ...
     * 快速添加
     */
    quickAddSchForMonth : function(){
    	
    	var that = this;
    	
    	var now =  new Date(this.startDate);
     	Ext.getCmp('year').setValue(now.getFullYear());
     	Ext.getCmp('month').setValue(now.getMonth());
    	
    	var cks = Ext.getCmp('checkboxgroup').getChecked(); 
    	var weeks = "";
        ...
        Ext.getCmp('weekStr').setValue(weeks.substring(0,weeks.length-1));
        if(Ext.getCmp('weekStr').getValue() == ''){
        	me.showAlert('选择星期..');
        	return ;
        }
    	...
    	Ext.Msg.show({
		    ...
		    fn: function(btn) {
		        if (btn === 'yes') {
		        	//sumit
		        	Ext.getCmp('quickAddSchFormPanel').getForm().submit({
						clientValidation: true,
						url : root + '/doctor/quickAddSchForMonth.freda',
						waitTitle : "",
						waitMsg : me.show_tips.wait_tips_msg,
						success: function(form, action) {
						   that.quickWin.close();
					       me.showAlert(action.result.msg);
					       that.setActiveView(that.id + '-month');
					    },
